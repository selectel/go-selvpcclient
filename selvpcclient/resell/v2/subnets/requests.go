...
func Get(ctx context.Context, client *selvpcclient.Client, id string) (*Subnet, *clientservices.ResponseResult, error) {
...
func List(ctx context.Context, client *selvpcclient.Client, opts ListOpts) ([]*Subnet, *clientservices.ResponseResult, error) {
	endpoint, err := client.Resell.GetEndpoint()
	if err != nil {
		return nil, nil, fmt.Errorf("failed to get endpoint, err: %w", err)
	}
...
func Create(ctx context.Context, client *selvpcclient.Client, projectID string, createOpts SubnetOpts) ([]*Subnet, *clientservices.ResponseResult, error) {
	endpoint, err := client.Resell.GetEndpoint()
	if err != nil {
		return nil, nil, fmt.Errorf("failed to get endpoint, err: %w", err)
	}
